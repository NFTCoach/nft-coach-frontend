...
export function Tournaments() {
  ...
  const getAttendedTournamentReq = useRequest(getAttendedTournament);
  ...
  useEffect(() => {
    if (!account.address && !attendedTournamentId) return;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await getAttendedTournamentReq.exec();
      console.log(res);
      setAttendedTournament(res);
    };
    fetchData();
  }, [account.isSignedIn]);
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={clsnm(styles["attended-tournament--container"])}>
          {tournamentStatus?.nextOpponent != null && (
            <Fragment>
              <Typography variant="title1">
                Next Match{" "}
                {moment(
                  new Date(new Date().getTime() + timeUntilNextMatch * 1000)
                ).from(new Date())}
              </Typography>
            </Fragment>
          )}
          ...
          <div className={styles["teams-container"]}>
            ...
            <div className={styles["team-stats--container"]}>
              ...
              <div className={styles["team-info"]}>
                ...
                <progress
                  max="100"
                  value={teamStats?.atkAvg.toString().slice(0, 2)}
                ></progress>
              </div>
              <div className={styles["team-info"]}>
                <label>Defence</label>
                ...
                <progress
                  max="100"
                  value={teamStats?.defAvg.toString().slice(0, 2)}
                ></progress>
              </div>
            </div>
        ...
        {getTournamentStatusReq.loading && (
          <div
            style={{
              height: "30vh",
              display: "flex",
