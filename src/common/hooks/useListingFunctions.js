...
export const useListingFunctions = () => {
  ...
  const listings = await Promise.all(listedIds.map(id =>
      contracts.Marketplace.idToListing(id).then((l) => new CardListing(id, l))
  ));
  ...
  const getAllPlayersOf = async (address) => {
    const mintEvents = await filterEvents(contracts.NC721, "Transfer", null, address);
    const playerIds = mintEvents.map(ev => ev.args[2].toString());

    let players = [];
    for (let playerId of playerIds) {
        const transferEvents = await filterEvents(contracts.NC721, "Transfer", null, null, ethers.BigNumber.from(playerId));
    ...
    const rentEvents = await filterEvents(contracts.Marketplace, "PlayerRented");
    const rentedIds = rentEvents.map(ev => ev.args[0].toString());
    for (let playerId of rentedIds) {
        if (await contracts.Management.idToCoach(playerId) == account.signer.address) {
            players.push(playerId);
        }
